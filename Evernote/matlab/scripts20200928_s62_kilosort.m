...
for ishank = 1:length(shanks_for_analysis)
    ...
    for ichan = 1:nchan_shank
        ...
        xcoords(mychan,1) = rem(ichan,2) * (-15) ...     % 左右15μm間隔。
                            + 1000 * ishank;             % シャンク間隔。本来150μmだけど大きめにとる
        ycoords(mychan,1) = -1 * ichan * 30;             % 上下30μmごと。 
...
load('s62map_kilosortChanMap', 'kcoords')
...
%%
figure
CCGm(spiketA,spikeindA, 200,50,20000, [20 384]);

figure
[ccg1,t] =CCGm(spiketB,spikeindB, 200,50,20000, [410 183]);
[ccg2,t] =CCGm(spiketB,spikeindB, 200,50,20000, [410 91]);
plot(t,ccg1/max(ccg1),t,ccg2/max(ccg2))
